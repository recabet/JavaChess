...
public class Table {
    ...
    private final BoardPanel boardPanel;// Panel to display the chess board
    ...
    private static final Dimension FRAME_DIMENSION = new Dimension(600, 600);// Dimension of the game frame
    private static final Dimension PANEL_BOARD_DIMENSION = new Dimension(400, 350);// Dimension of the board panel
    private static final Dimension PANEL_SQUARE_DIMENSION = new Dimension(10, 10);// Dimension of each square on the board
    ...
    public Table() throws IOException
    {
        ...
        JFrame gameFrame = new JFrame("Java Chess");
    ...
    private class SquarePanel extends Panel {
                ...
                public void mouseClicked(MouseEvent e)
                {
                    ...
                    } else if(isLeftMouseButton(e))
                    {
                        ...
                        } else
                        {
                            ...
                            final Move move = Move.AttackMove.MoveCreator.moveCreate(chessBoard, sourceSquare.getCoord(), destinationSquare.getCoord());
                            ...
                            if(changer.moveSt().isDone())
                            {
                                chessBoard = changer.changedBoard();
        ...
        private void assignSquarePiece(final Board board)
        {
            ...
            if(board.getSquare(this.SquareId).isOccupied())
            {
                try
                {
                    // Path to the directory containing piece icons
                    String pieceIconPath = "images/fancy/";
                    final BufferedImage image = ImageIO.read(new File(pieceIconPath + board.getSquare(this.SquareId).getPiece().getPieceColor().toString().charAt(0) + board.getSquare(this.SquareId).getPiece().toString() + ".gif"));
        ...
        {
            if(BoardData.EIGHTH_RANK[this.SquareId] || BoardData.SIXTH_RANK[this.SquareId] || BoardData.FOURTH_RANK[this.SquareId] || BoardData.SECOND_RANK[this.SquareId])
            {
            ...
            } else if(BoardData.SEVENTH_RANK[this.SquareId] || BoardData.FIFTH_RANK[this.SquareId] || BoardData.THIRD_RANK[this.SquareId] || BoardData.FIRST_RANK[this.SquareId])
            {
                if(this.SquareId % 2 != 0)
                {
                    setBackground(lightTileColor);
